    ...
    case QAudio::AudioOutput: {
        ...
        if (m_file) {
            if (levelPosition > m_bufferPosition ||
                ...
                const qint64 readLen = readEnd - readPos + Utils::audioLength(m_format, WaveformWindowDuration);
...
void Engine::calculateLevel(qint64 position, qint64 length)
{
    ...
    while (ptr < end) {
        ...
        const qreal fracValue = Utils::pcmToReal(value);
...
void Engine::setFormat(const QAudioFormat& format)
{
    ...
    m_format = format;
    m_levelBufferLength = Utils::audioLength(m_format, LevelWindowUs);
